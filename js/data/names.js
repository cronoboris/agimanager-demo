...
export const PERSONALITIES = data.PERSONALITIES || [];
export const IDEOLOGIES = data.IDEOLOGIES || [];
...
export function generateTalent(country = 'us', level = null) {
    ...
    const personality = PERSONALITIES[Math.floor(Math.random() * PERSONALITIES.length)] || null;
    const ideologyProfile = IDEOLOGIES[Math.floor(Math.random() * IDEOLOGIES.length)] || null;
    const ideology = ideologyProfile?.id || (Math.random() < 0.3 ? 'safety' : (Math.random() < 0.5 ? 'accel' : 'neutral'));
