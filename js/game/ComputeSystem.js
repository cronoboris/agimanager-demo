...
function _normalizeDatacenterEffects(datacenter = {}) {
    const location = DATACENTER_LOCATIONS?.[datacenter.locationId || 'domestic'] || DATACENTER_LOCATIONS?.domestic || {};
    datacenter.constructionEvents = Array.isArray(datacenter.constructionEvents) ? datacenter.constructionEvents : [];
    datacenter.operationalEvents = Array.isArray(datacenter.operationalEvents) ? datacenter.operationalEvents : [];
    datacenter.activeOperationalEffects = Array.isArray(datacenter.activeOperationalEffects)
        ? datacenter.activeOperationalEffects
            .map(effect => ({
                ...effect,
                tflopsLoss: Math.max(0, _safeNumber(effect?.tflopsLoss, 0)),
                remainingMonths: Math.max(0, Math.round(_safeNumber(effect?.remainingMonths, effect?.duration ?? 0)))
            }))
            .filter(effect => effect.remainingMonths > 0 || effect.tflopsLoss > 0)
        : [];
    datacenter.bonuses = {
        ...(location?.bonuses || {}),
        ...(datacenter.bonuses && typeof datacenter.bonuses === 'object' ? datacenter.bonuses : {})
    };
    const rawRisks = datacenter.risks && typeof datacenter.risks === 'object' ? datacenter.risks : {};
    datacenter.risks = {
        disasterRisk: Math.max(0, _safeNumber(rawRisks.disasterRisk, location?.disasterRisk ?? 0)),
        regulationRisk: Math.max(0, _safeNumber(rawRisks.regulationRisk, location?.regulationRisk ?? 0))
    };
    datacenter.locationId ||= 'domestic';
    datacenter.id ||= datacenter.datacenterId || null;
    datacenter.powerCostMult = _safeNumber(
        datacenter.powerCostMult
        ?? location?.powerCostMult,
        1
    );
    datacenter.gpus = Math.max(0, Math.round(_safeNumber(datacenter.gpus, 0)));
    datacenter.monthlyCost = Math.max(0, Math.round(_safeNumber(datacenter.monthlyCost, 0)));
    datacenter.buildMonthsTotal = Math.max(0, Math.round(_safeNumber(datacenter.buildMonthsTotal, datacenter.buildMonthsLeft ?? 0)));
    datacenter.buildMonthsLeft = Math.max(0, Math.round(_safeNumber(datacenter.buildMonthsLeft, 0)));
    datacenter.operational = Boolean(datacenter.operational || datacenter.buildMonthsLeft <= 0);
    if (datacenter.operational) datacenter.buildMonthsLeft = 0;
    return datacenter;
}

function _resolveDatacenters(options = {}) {
    if (Array.isArray(options.datacenters)) return options.datacenters;
    const liveDatacenters = globalThis?.window?.game?.state?.economy?.datacenters;
    return Array.isArray(liveDatacenters) ? liveDatacenters : [];
}

function _getDatacenterThroughputMultiplier(datacenter = {}) {
    const totalLoss = (datacenter?.activeOperationalEffects || []).reduce((sum, effect) => {
        return sum + Math.max(0, _safeNumber(effect?.tflopsLoss, 0));
    }, 0);
    return Math.max(0.05, 1 - totalLoss);
}

function _assignDatacenterMetadataToFleet(fleet = [], datacenters = []) {
    const operationalDatacenters = datacenters
        .filter(datacenter => datacenter?.operational && datacenter?.gpus > 0)
        .map(datacenter => _normalizeDatacenterEffects(datacenter))
        .sort((a, b) => _safeNumber(a.powerCostMult, 1) - _safeNumber(b.powerCostMult, 1) || b.gpus - a.gpus);

    if (operationalDatacenters.length === 0) return _mergeGpuSlots(fleet);

    const capacityUsed = new Map(operationalDatacenters.map(datacenter => [datacenter.id, 0]));
    for (const slot of fleet) {
        if (!slot || slot.location !== 'datacenter' || !slot.datacenterId || !capacityUsed.has(slot.datacenterId)) continue;
        capacityUsed.set(slot.datacenterId, capacityUsed.get(slot.datacenterId) + Math.max(0, Number(slot.count || 0)));
    }

    const resolvedSlots = [];
    for (const slot of fleet) {
        if (!slot || slot.location !== 'datacenter') {
            resolvedSlots.push(slot);
            continue;
        }

        const existingDatacenter = operationalDatacenters.find(datacenter => datacenter.id === slot.datacenterId);
        if (existingDatacenter) {
            resolvedSlots.push({
                ...slot,
                datacenterId: existingDatacenter.id,
                datacenterLocationId: existingDatacenter.locationId,
                datacenterPowerCostMult: _safeNumber(existingDatacenter.powerCostMult, 1)
            });
            continue;
        }

        let remaining = Math.max(0, Math.round(_safeNumber(slot.count, 0)));
        for (const datacenter of operationalDatacenters) {
            const used = capacityUsed.get(datacenter.id) || 0;
            const free = Math.max(0, datacenter.gpus - used);
            if (free <= 0 || remaining <= 0) continue;
            const assigned = Math.min(remaining, free);
            capacityUsed.set(datacenter.id, used + assigned);
            resolvedSlots.push({
                ...slot,
                count: assigned,
                datacenterId: datacenter.id,
                datacenterLocationId: datacenter.locationId,
                datacenterPowerCostMult: _safeNumber(datacenter.powerCostMult, 1)
            });
            remaining -= assigned;
        }

        if (remaining > 0) {
            const fallback = operationalDatacenters[operationalDatacenters.length - 1];
            resolvedSlots.push({
                ...slot,
                count: remaining,
                datacenterId: fallback.id,
                datacenterLocationId: fallback.locationId,
                datacenterPowerCostMult: _safeNumber(fallback.powerCostMult, 1)
            });
        }
    }

    return _mergeGpuSlots(resolvedSlots);
}
...
export function getServiceRevenueMultiplier(relativePerformance) {
    const rel = _safeNumber(relativePerformance, 0);
    if (rel < 0.5) return 0.35 + (rel * 0.4);
    if (rel < 1.0) return 0.55 + ((rel - 0.5) * 0.9);
    if (rel < 1.5) return 1.0 + ((rel - 1.0) * 1.25);
    return 1.625 + ((rel - 1.5) * 0.45);
}

export function calculateModelEvaluationAxes(model = {}) {
    const capabilities = model.capabilities || {};
    const language = _safeNumber(capabilities.language, _safeNumber(model.performance ?? model.compositeScore, 0));
    const reasoning = _safeNumber(capabilities.reasoning, language);
    const coding = _safeNumber(capabilities.coding, Math.max(10, language * 0.7));
    const multimodal = _safeNumber(capabilities.multimodal, Math.max(5, language * 0.4));
    const safety = _safeNumber(capabilities.safety, _safeNumber(model.safetyScore, 40));
    const speed = _safeNumber(capabilities.speed, 50);

    return {
        capability: Math.round(_clamp((language + reasoning + coding) / 3, 0, 100)),
        reasoning: Math.round(_clamp(reasoning, 0, 100)),
        coding: Math.round(_clamp(coding, 0, 100)),
        safety: Math.round(_clamp(safety, 0, 100)),
        latency: Math.round(_clamp(speed, 0, 100)),
        multimodal: Math.round(_clamp(multimodal, 0, 100))
    };
}

const SERVICE_CHANNEL_FIT_WEIGHTS = {
    api: { capability: 0.3, reasoning: 0.2, coding: 0.1, safety: 0.1, latency: 0.2, multimodal: 0.1 },
    consumer_chat: { capability: 0.3, reasoning: 0.15, coding: 0.05, safety: 0.1, latency: 0.3, multimodal: 0.1 },
    enterprise: { capability: 0.2, reasoning: 0.2, coding: 0.2, safety: 0.15, latency: 0.15, multimodal: 0.1 },
    government: { capability: 0.15, reasoning: 0.15, coding: 0.05, safety: 0.4, latency: 0.1, multimodal: 0.15 },
    open_source: { capability: 0.25, reasoning: 0.2, coding: 0.2, safety: 0.05, latency: 0.1, multimodal: 0.2 },
    internal: { capability: 0.2, reasoning: 0.25, coding: 0.15, safety: 0.2, latency: 0.15, multimodal: 0.05 },
    b2b_license: { capability: 0.2, reasoning: 0.15, coding: 0.25, safety: 0.15, latency: 0.1, multimodal: 0.15 }
};

export function getServiceChannelFitScore(model = {}, channelType = 'api') {
    const axes = calculateModelEvaluationAxes(model);
    const weights = SERVICE_CHANNEL_FIT_WEIGHTS[channelType] || SERVICE_CHANNEL_FIT_WEIGHTS.api;
    const weighted = Object.entries(weights)
        .reduce((sum, [axisId, weight]) => sum + (_safeNumber(axes[axisId], 0) * _safeNumber(weight, 0)), 0);
    return _clamp(weighted / 100, 0, 1);
}
...
function _normalizeGpuSlot(slot, options = {}) {
    ...
    if (source === 'colocation') {
        ...
        location = options.defaultOwnedLocation || 'warehouse';
    }
    if (!['cloud', 'colocation', 'datacenter', 'warehouse'].includes(location)) {
        location = source === 'cloud' ? 'cloud' : (options.defaultOwnedLocation || 'warehouse');
    }
...
export function getFleetStats(fleet, options = {}) {
    ...
    const datacenters = _resolveDatacenters(options).map(datacenter => _normalizeDatacenterEffects(datacenter));

    function _resolveDatacenterForSlot(slot) {
        if (slot?.datacenterId) {
            const byId = datacenters.find(datacenter => datacenter && (datacenter.id === slot.datacenterId || datacenter.datacenterId === slot.datacenterId));
            if (byId) return _normalizeDatacenterEffects(byId);
        }
        if (slot?.datacenterLocationId) {
            const found = datacenters.find(datacenter => datacenter && datacenter.locationId === slot.datacenterLocationId) || null;
            return found ? _normalizeDatacenterEffects(found) : null;
        }
        return null;
    }
    ...
    for (const slot of normalizedFleet) {
        ...
        const datacenter = locationKey === 'datacenter' ? _resolveDatacenterForSlot(slot) : null;
        const throughputMult = locationKey === 'datacenter'
            ? _getDatacenterThroughputMultiplier(datacenter)
            : 1;
        const isActive = locationKey === 'cloud' || locationKey === 'colocation' || locationKey === 'datacenter';
        if (isActive) {
            stats.activeCount += count;
            stats.totalTFLOPS += _safeNumber(resolved.tflops, 0) * count * throughputMult;
        ...
        if (slot.source === 'owned') {
            ...
            if (locationKey === 'warehouse') {
                ...
                if (resolved.monthlyOwnedCost != null) {
                    ownedCost = _safeNumber(resolved.monthlyOwnedCost, 0);
                } else if (electricityCostPerKwh > 0 && resolved.powerWatt != null) {
                    ownedCost = (resolved.powerWatt / 1000) * 24 * 30 * electricityCostPerKwh;
                } else {
                    ownedCost = BALANCE.ECONOMY.GPU_POWER_COST;
                }

                if (locationKey === 'datacenter') {
                    const datacenterPowerCostMult = _safeNumber(
                        slot.datacenterPowerCostMult
                        ?? datacenter?.powerCostMult
                        ?? DATACENTER_LOCATIONS?.[datacenter?.locationId || slot?.datacenterLocationId]?.powerCostMult,
                        1
                    );
                    ownedCost *= datacenterPowerCostMult;
                }

                stats.monthlyOwnedCost += ownedCost * count;
...
function _ensureEconomyAccessors(economy, resources, currentYear = 2017) {
    ...
    let gpuFleet = normalizeGpuFleet(economy.gpuFleet ?? {
        ...
        cloudGPUs,
        currentYear
    }, { currentYear, defaultOwnedLocation: 'warehouse' });
    ...
    const syncFleetFromCounts = () => {
        gpuFleet = normalizeGpuFleet({
            ...
            currentYear,
            defaultOwnedLocation: 'warehouse',
        ...
        const stats = getFleetStats(gpuFleet, {
            ...
            datacenters: economy.datacenters
        });
...
export function syncStateEconomyCompatibility(state, currentYear = 2017) {
    ...
    state.economy.pendingTermSheets = Array.isArray(state.economy.pendingTermSheets) ? state.economy.pendingTermSheets : null;
    state.economy.termSheetDeadline = Math.max(0, Number(state.economy.termSheetDeadline || 0));
    if (Array.isArray(state.economy.datacenters)) {
        state.economy.datacenters = state.economy.datacenters.map((datacenter, index) => {
            if (!datacenter || typeof datacenter !== 'object') return null;
            datacenter.id ||= `dc-${index + 1}`;
            datacenter.tierId ||= 'medium';
            datacenter.locationId ||= 'domestic';
            datacenter.name ||= datacenter.nameKey || datacenter.locationName || `Datacenter ${index + 1}`;
            datacenter.gpus = Math.max(0, Number(datacenter.gpus || 0));
            datacenter.monthlyCost = Math.max(0, Number(datacenter.monthlyCost || 0));
            datacenter.buildMonthsTotal = Math.max(0, Number(datacenter.buildMonthsTotal || datacenter.buildMonths || 0));
            datacenter.buildMonthsLeft = Math.max(0, Number(datacenter.buildMonthsLeft ?? datacenter.buildMonthsTotal ?? 0));
            datacenter.operational = Boolean(datacenter.operational);
            _normalizeDatacenterEffects(datacenter);
            return datacenter;
        }).filter(Boolean);
    }
    ...
    const fleetStatsBeforeSync = getFleetStats(state.economy.gpuFleet, {
        ...
        datacenters: state.economy.datacenters
    });
    ...
    // Migrate legacy saves: owned GPU slots without explicit location
    // If operational datacenters exist, keep them as 'datacenter'; otherwise 'warehouse'
    if (Array.isArray(fleetSeed)) {
        const datacenters = state.economy.datacenters || [];
        const hasOperationalDC = datacenters.some(dc => dc && dc.operational);
        for (const slot of fleetSeed) {
            if (slot && slot.source === 'owned' && !slot.location) {
                slot.location = hasOperationalDC ? 'datacenter' : 'warehouse';
            }
        }
    }
    ...
    // Clamp datacenter GPU count to actual capacity (fix old auto-fill bug)
    const dcCapacity = (state.economy.datacenters || [])
        .filter(dc => dc && dc.operational)
        .reduce((sum, dc) => sum + Math.max(0, Number(dc.gpus || 0)), 0);
    const dcFleetCount = state.economy.gpuFleet
        .filter(slot => slot.source === 'owned' && slot.location === 'datacenter')
        .reduce((sum, slot) => sum + (slot.count || 0), 0);
    if (dcFleetCount > dcCapacity) {
        let overflow = dcFleetCount - dcCapacity;
        for (const slot of state.economy.gpuFleet) {
            if (slot.source === 'owned' && slot.location === 'datacenter' && overflow > 0) {
                const move = Math.min(slot.count, overflow);
                slot.count -= move;
                overflow -= move;
                state.economy.gpuFleet.push({
                    gpuId: slot.gpuId, count: move,
                    source: 'owned', location: 'warehouse',
                    name: slot.name, releaseYear: slot.releaseYear
                });
            }
        }
        state.economy.gpuFleet = state.economy.gpuFleet.filter(s => s.count > 0);
        state.economy.gpuFleet = normalizeGpuFleet(state.economy.gpuFleet, {
            currentYear, defaultOwnedLocation: 'warehouse', completedChipPrograms, customGpus
        });
    }

    state.economy.gpuFleet = _assignDatacenterMetadataToFleet(
        state.economy.gpuFleet,
        state.economy.datacenters || []
    );
